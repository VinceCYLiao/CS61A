...
def extra_turn(player_score, opponent_score):
    ...
    return (more_boar(player_score, opponent_score) or
...
def more_boar(player_score, opponent_score):
    """Return whether the player gets an extra turn due to More Boar.
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while(True):
        ...
        if rolls > 10:
                rolls = 10
        outcome = take_turn(rolls, opponent_score, dice)
        
        """calculate score0 and score1"""
        if (who == 0):
            score0 = score0 + outcome
        else:
        ...
        """check extra turn"""
        if (who == 0):
            p_score, o_score = score0, score1
            if not extra_turn(p_score, o_score):
                who = other(who)
        else:
            p_score, o_score = score1, score0
            if not extra_turn(p_score, o_score):
                who = other(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(s0, s1):
        ...
        print("DEBUG:", s0, s1, score, diff, running_high)
        if diff > running_high:
            print(diff, "point(s)! The most yet for Player", who)
        high = diff if diff > running_high else running_high
